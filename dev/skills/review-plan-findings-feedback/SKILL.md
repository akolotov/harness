...
```bash
bash scripts/new_findings_dir.sh <plan-id>
```
...
The script anchors `.ai/impl_plans` at the git repository root. If the project keeps
implementation plans elsewhere, pass that directory as the optional second argument.
...
- Relevant code, tests, and docs needed to verify whether the feedback and plan edits are correct.
- Agent-facing rule files, only if the repository has them (e.g. `.cursor/rules/*.mdc`, `CLAUDE.md`, `AGENTS.md`).
...
- `../implementation-plan-review/references/finding-adjudication-protocol.md`
- `../implementation-plan-review/assets/finding-adjudication-report.md`

Create a fresh adjudication run:

```bash
bash ../implementation-plan-review/scripts/new_scratchpads_dir.sh \
...
limited. Give each subagent no conversation history, only the launcher prompt,
and prefer a strong reasoning model with the highest available reasoning effort
unless the user requests otherwise. The launcher prompt must contain only:
...
```bash
python3 ../implementation-plan-review/scripts/verify_adjudication_run.py \
